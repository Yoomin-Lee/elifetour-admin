...
ALTER TABLE mn_sections ADD COLUMN IF NOT EXISTS deleted_at timestamptz;

-- RLS 정책 업데이트: deleted_at NULL인 행만 일반 조회
DROP POLICY IF EXISTS "auth_read_mn_sections" ON mn_sections;
CREATE POLICY "auth_read_mn_sections" ON mn_sections FOR SELECT USING (auth.role() = 'authenticated');

-- 누락된 원본 데이터 복원 (title+category 중복 없는 경우만 삽입)
INSERT INTO mn_sections (category, title, description, row_type, rows, sort_order)
SELECT vals.category, vals.title, vals.description, vals.row_type, vals.rows::jsonb, vals.sort_order
FROM (VALUES
  ('취소료', '코스타 취소료 규정 (2025~)',
  ...
  ('팁', '로얄 캐리비안 팁',
   ...
   3)
) AS vals(category, title, description, row_type, rows, sort_order)
WHERE NOT EXISTS (
  SELECT 1 FROM mn_sections
  WHERE category = vals.category AND title = vals.title AND deleted_at IS NULL
);
